...
export const useRouteHandler = () => {
  ...
  const updateSearchParams = useCallback((search: SearchParam) => {
    !search.value
      ? params.delete(search.name)
      : params.set(search.name, search.value.toLowerCase())
  }, [params]);

  const getUrl = useCallback(({ search }: RouteHandlerParams) => {
  ...
  const replaceUrl = useCallback(({ search, options }: RouteHandlerParams) => {
    router.replace(getUrl({ search }), options)
  }, [getUrl, router])

  const pushUrl = useCallback(({ search, options }: RouteHandlerParams) => {
